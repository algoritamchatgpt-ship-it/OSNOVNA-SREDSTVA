...
public partial class LoginViewModel : ObservableObject
{
    ...
    private async Task PrijavaAsync()
    {
        ...
        try
        {
            ...
            if (korisnik != null)
            {
                if (!_loginSessionService.TryAcquireSession(korisnik.KorisnikIme, out var lockHandle, out var lockMsg))
                {
                    Poruka = string.IsNullOrWhiteSpace(lockMsg)
                        ? $"Korisnik '{korisnik.KorisnikIme}' je već prijavljen."
                        : lockMsg;
            ...
            {
                Poruka = "Pogrešan korisnik ili lozinka.";
                Lozinka = string.Empty;
            }
        }
        finally
